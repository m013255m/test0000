...
export default function App() {
  ...
  const handleQuickScanCode = (code: string) => {
    ...
    const legacyMatch = cleanCode.match(/^ps-mem-(\d{1,7})$/i);
    const plainMatch = cleanCode.match(/^(\d{1,7})$/);
    const numericCode = legacyMatch?.[1] || plainMatch?.[1] || '';
    const normalizedPublicId = numericCode ? numericCode.padStart(7, '0') : '';

    const member = members.find(
      (m) =>
        m.qrCodeValue === normalizedPublicId ||
        m.qrCodeValue.toLowerCase() === cleanCode ||
        m.id.toLowerCase() === cleanCode ||
        m.fullName.toLowerCase() === cleanCode
    );
