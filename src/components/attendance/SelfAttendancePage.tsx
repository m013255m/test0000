...
export const SelfAttendancePage: React.FC<SelfAttendancePageProps> = ({
  ...
  useEffect(() => {
    ...
    // Fetch latest members synced from computer over LAN
    const fetchLanMembers = async () => {
      try {
  ...
  const findMember = (query: string): Member | undefined => {
    ...
    return members.find((m) => {
      // Direct ID match (e.g. "mem-101")
      if (m.id.toLowerCase() === cleanQuery) return true;

      // Numeric ID match (e.g. member enters "101" instead of "mem-101")
      if (m.id.replace('mem-', '').toLowerCase() === cleanQuery) return true;

      // QR Code Value match (e.g. "PS-MEM-101")
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder="مثال: mem-101 أو 101 أو رقم الهاتف"
