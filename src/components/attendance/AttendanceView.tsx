...
import { normalizeMemberQrCode, storageService, getTodayDateString } from '../../services/storage';
...
export const AttendanceView: React.FC<AttendanceViewProps> = ({ tenantId }) => {
  ...
  useEffect(() => {
    ...
    // 2. Sync attendance records from Firebase so check-ins made from the
    // public/self-attendance page on another device appear in this dashboard.
    const syncCloudAttendance = async () => {
      try {
        const cloudAttendance = await firebaseService.fetchAttendance(tenantId);
        if (!Array.isArray(cloudAttendance)) return;

        const currentLocal = storageService.getAttendance(tenantId);
        const byId = new Map(currentLocal.map((record) => [record.id, record]));
        let hasChanges = false;

        cloudAttendance.forEach((cloudRecord) => {
          if (cloudRecord.tenantId !== tenantId || !cloudRecord.id) return;
          const existing = byId.get(cloudRecord.id);
          if (!existing || cloudRecord.checkInTimestamp > existing.checkInTimestamp) {
        ...
        if (hasChanges) {
          ...
          storageService.saveAttendance(merged, tenantId);
          setAttendance(merged);
        }
      } catch {
        // Keep working from local/LAN data when Firebase is unavailable.
      }
    };

    syncCloudAttendance();

    // 3. Poll LAN attendance from phones
  ...
  const handleScanCode = (code: string) => {
    const member = members.find(
      (m) =>
        normalizeMemberQrCode(m.qrCodeValue) === normalizeMemberQrCode(code) ||
